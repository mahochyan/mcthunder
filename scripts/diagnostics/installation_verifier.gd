extends Node
## Optional bounded installation diagnostic, invoked only by --verify-installation.
var checks := 0
var failed := 0
var app: AppFlow
func inventory(path: String) -> Array[String]:
	var found: Array[String] = []
	for file in DirAccess.get_files_at(path): found.append(path.path_join(file))
	for folder in DirAccess.get_directories_at(path): found.append_array(inventory(path.path_join(folder)))
	return found
...
func run(flow: AppFlow) -> void:
	...
	for file in inventory("res://"):
		var relative := file.trim_prefix("res://")
		if relative.get_slice("/",0) not in [".godot","assets","configs","scenes","scripts","icon.svg","icon.svg.import","project.binary"]: unexpected.append(relative)
		if relative.get_extension() in ["blend","py","ps1","zip","exe","bat"]: unexpected.append(relative)
	check(unexpected.is_empty(),"T031-02 complete PCK inventory excludes non-runtime files: "+str(unexpected))
	...
	# AppFlow prepared isolated settings and profile before creating this diagnostic.
	var profile_path := app.profile._path
	...
	check(ProfileStore.new(profile_path).snapshot().tutorial.chapter==0,"T031-01 chapter checkpoint reopens from actual disk")
