...
func natural_matches() -> void:
	...
	for map_index in 2:
		...
		if frontend.collection_scroll != null:
			frontend.collection_scroll.ensure_control_visible(frontend.cards[vehicle_index])
		...
		if not selected:
			print("[garage card failure] wanted=",wanted," actual=",app.garage.selected_vehicle_id()," rect=",frontend.cards[vehicle_index].get_global_rect())
			await capture("failed_vehicle_card_"+str(map_index))
