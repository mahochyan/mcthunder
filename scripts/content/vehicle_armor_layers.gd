...
static func check(packet: Dictionary) -> Array[String]:
	...
	for row in layers:
		...
		for key in row:
			if key not in ["id","part","vertices_m","normal","thickness_mm","material","response_profile","reason"]: errors.append(path+": unsupported field "+str(key))
		if not row.get("id") is String or not str(row.get("id","")).is_valid_identifier() or ids.has(row.get("id")): errors.append(path+": unique identifier required")
...
static func install_bound_visuals(actor: VehicleActor, packet: Dictionary, layout: VehicleLayoutDefinition) -> void:
	...
	for patch in layout.armor_patches:
		if not ids.has(patch.id): continue
