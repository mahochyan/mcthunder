...
func setup(parent: TankVehicle, packet: Dictionary) -> void:
	...
	for side in [-1,1]:
		var frame := tank.track_left_frame if side<0 else tank.track_right_frame
		var mesh:=frame.get_node_or_null("Cosmetic_track_left" if side<0 else "Cosmetic_track_right") as MeshInstance3D
