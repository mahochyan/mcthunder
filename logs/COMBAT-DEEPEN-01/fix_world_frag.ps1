...
# (1) an external blast is an OUTSIDE explosion, so it needs the multi-target eligibility the delayed case already has
...
"  gate_var=$n1 ; eligibility=$n2 ; guard=$n3 ; leave=$n4"

& $g --headless --path $c --check-only --script res://scripts/projectiles/fragment_system.gd *> "$L\parse-fs.log" 2>&1 | Out-Null
$err=@(Get-Content "$L\parse-fs.log" | Select-String 'Parse Error|Compile Error').Count
"  fragment_parse_errors=$err"
if ($err -gt 0) { Get-Content "$L\parse-fs.log" | Select-String 'Parse Error|at:' | Select-Object -First 5 | ForEach-Object { "   " + $_.Line.Trim().Substring(0,[Math]::Min(175,$_.Line.Trim().Length)) }; exit 1 }
