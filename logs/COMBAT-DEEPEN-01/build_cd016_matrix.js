...
const unclassified = migration.changes.map((c) => c.new_rule_version).filter((v) => !LEVEL[v]);
...
const out = {
  ...
  template_only: false,
  purpose: 'Filled from what the fifteen closed sub-orders MEASURED, not from what they intended. It contains ZERO WT_BEHAVIOR_COMPARISON rows and ZERO HUMAN_PLAYTEST rows, gives NO similarity percentage, and leaves the War Thunder half of every row null and NOT_COMPARED because no capture of any external title exists in this work.',
  ...
  summary: {
    ...
    note: 'A count of rows or a count of passes is NOT a similarity and is not offered as one. The only honest statement this matrix makes about external behaviour is that no comparison has been made.'
  },
  dependency_graph: {
...
must(rows.every((r) => r.expected_reference === null && r.tolerance_predeclared === null), 'no row invents an external reference or tolerance');
must(rows.every((r) => r.war_thunder.game_build === null && r.war_thunder.source_url_or_capture_id === null), 'the War Thunder half stays empty');
must(!rows.some((r) => r.outcome === 'WT_BEHAVIOR_COMPARISON' || r.outcome === 'HUMAN_PLAYTEST'), 'no row claims an external or human level');
...
md.push('');
md.push('Filled from what the fifteen closed sub-orders measured. It contains **zero** `WT_BEHAVIOR_COMPARISON` rows and **zero** `HUMAN_PLAYTEST` rows, gives **no** similarity percentage, and leaves the War Thunder half of every row `null` and `NOT_COMPARED` because no capture of any external title exists in this work.');
md.push('');
...
md.push('|---|---|---|---|---|---|');
for (const r of rows) {
  md.push('| `' + r.id + '` | ' + r.work_order_id + ' | `' + r.outcome + '` | ' + r.mechanic.replace(/\|/g, '/') + ' | ' + r.actual_project.replace(/\|/g, '/').slice(0, 260) + (r.actual_project.length > 260 ? ' …' : '') + ' | `null` / NOT_COMPARED |');
}
md.push('');
...
md.push('|---|---|');
md.push('| `war_thunder.game_build` | `null` in every row |');
md.push('| `war_thunder.mode` | `' + mode + '` |');
md.push('| `war_thunder.vehicle_variant` / `shell` / `crew_or_modifications` | `null` in every row |');
md.push('| `war_thunder.source_url_or_capture_id` / `observed_at` | `null` in every row |');
