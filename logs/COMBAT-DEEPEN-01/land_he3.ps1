...
# (0) the one line the external blast needs in the impact-profile whitelist
...
if ($n0 -eq 1) { $t0 = $t0.Replace($a0,$b0); [IO.File]::WriteAllText($p0, $t0, (New-Object Text.UTF8Encoding($false))) }
"  whitelist_edit=$n0"
...
$j = Start-Job -ScriptBlock { param($g,$c,$L) & $g --headless --path $c --fixed-fps 60 -s res://tests/probe_cd007_he_admission.gd *> "$L\hadm5.log" } -ArgumentList $g,$c,$L
$d = Wait-Job $j -Timeout 300; if (-not $d) { Stop-Job $j }; Remove-Job $j
$log = Get-Content "$L\hadm5.log" -Encoding UTF8 -ErrorAction SilentlyContinue
"=== diagnostic ==="
...
if ($buildOk) {
  ...
  foreach ($s in 'run_modern_garage_checks','run_modern_armor_frame_checks','run_shell_checks','run_historical_checks') {
    & $g --headless --path $c --fixed-fps 60 -s ("res://tests/" + $s + ".gd") *> "$L\he7-$s.log"
    $o = Get-Content "$L\he7-$s.log" -ErrorAction SilentlyContinue
