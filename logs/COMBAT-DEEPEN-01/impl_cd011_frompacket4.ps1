...
'=== (b) append the packet driven override AFTER the last match arm, which ends the function ==='
...
$vout = @()
for ($i=0; $i -le $vi; $i++) { $vout += $vl[$i] }
$vout += $vadd
for ($i=$vi+1; $i -lt $vl.Count; $i++) { $vout += $vl[$i] }
[IO.File]::WriteAllLines($vp, $vout, (New-Object Text.UTF8Encoding($false)))
'  pipeline lines ' + $v0 + ' -> ' + $vout.Count + ' (expect +' + $vadd.Count + ')'

'=== SHAPE FIRST ==='
& $g --headless --path $c --import *> "$L\impR.log" 2>&1 | Out-Null
$bad = 0
foreach ($f in @('res://scripts/drive/drive_profile.gd','res://scripts/content/vehicle_content_pipeline.gd')) {
  & $g --headless --path $c --check-only --script $f *> "$L\sh16.log" 2>&1 | Out-Null
  $e = @(Get-Content "$L\sh16.log" | Select-String 'Parse Error|Compile Error').Count
  '  ' + $f + ' parse_errors=' + $e
  if ($e -gt 0) { $bad += 1; Get-Content "$L\sh16.log" | Select-String 'Parse Error|at:' | Select-Object -First 4 | ForEach-Object { '      ' + $_.Line.Trim().Substring(0,[Math]::Min(180,$_.Line.Trim().Length)) } }
}
...
'  shape good for both'
'=== the legacy path must be intact: the admission diagnostic ==='
$j = Start-Job -ScriptBlock { param($g,$c,$L) & $g --headless --path $c --fixed-fps 60 -s res://tests/diag_cd011_admission.gd *> "$L\diagrun4.log" } -ArgumentList $g,$c,$L
$d = Wait-Job $j -Timeout 300; if (-not $d) { Stop-Job $j } ; Remove-Job $j
Start-Sleep -Milliseconds 300
Get-Content "$L\diagrun4.log" -Encoding UTF8 -ErrorAction SilentlyContinue | Select-String 'load_engineering ok|setup ok' | Select-Object -First 6 | ForEach-Object { '  ' + $_.Line.Trim().Substring(0,[Math]::Min(200,$_.Line.Trim().Length)) }
'=== and the CD11 scenes ==='
$jr = Start-Job -ScriptBlock { param($g,$c,$L) & $g --headless --path $c --fixed-fps 60 -s res://tests/run_cd011_scene_checks.gd *> "$L\c11g.log" } -ArgumentList $g,$c,$L
$d2 = Wait-Job $jr -Timeout 400; if (-not $d2) { Stop-Job $jr } ; Remove-Job $jr
Start-Sleep -Milliseconds 400
$o2 = Get-Content "$L\c11g.log" -Encoding UTF8 -ErrorAction SilentlyContinue
'  errors=' + @($o2|Select-String 'SCRIPT ERROR').Count
$o2 | Select-String 'CD11-T0|not_yet_met=|CD11_SCENES|^\[FAIL\]' | ForEach-Object { '  ' + $_.Line.Trim().Substring(0,[Math]::Min(215,$_.Line.Trim().Length)) }
