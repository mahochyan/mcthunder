...
function Build-He($v) {
  ...
  # An external HE bursts ON CONTACT, and the fuze contract accepts a delay fuze only for an internal burst shell, which is
  # exactly what the order's third design point says: contact HE first, proximity and timed fuzes are out of this version.
...
$fixture = Build-He $real
[IO.File]::WriteAllText("$fix\ussr_t_80b_he.json", ($fixture | ConvertTo-Json -Depth 12), (New-Object Text.UTF8Encoding($false)))
$j = Start-Job -ScriptBlock { param($g,$c,$L) & $g --headless --path $c --fixed-fps 60 -s res://tests/probe_cd007_he_admission.gd *> "$L\hadm4.log" } -ArgumentList $g,$c,$L
$d = Wait-Job $j -Timeout 300; if (-not $d) { Stop-Job $j }; Remove-Job $j
$log = Get-Content "$L\hadm4.log" -Encoding UTF8 -ErrorAction SilentlyContinue
"=== diagnostic (fuze removed) ==="
$log | Select-String 'CD07 HE diagnostic' | ForEach-Object { "  " + $_.Line.Trim().Substring(0,[Math]::Min(205,$_.Line.Trim().Length)) }
$buildOk = [bool]($log | Select-String 'VehicleShellCatalog.build ok=true')
"  buildOk=$buildOk"
if ($buildOk) {
  "=== clean, landing ==="
  $landed = Build-He $real
  ...
  foreach ($s in 'run_modern_garage_checks','run_modern_armor_frame_checks','run_shell_checks','run_historical_checks') {
    & $g --headless --path $c --fixed-fps 60 -s ("res://tests/" + $s + ".gd") *> "$L\he6-$s.log"
    $o = Get-Content "$L\he6-$s.log" -ErrorAction SilentlyContinue
...
} else { "  still not clean, delivered packet untouched" }
