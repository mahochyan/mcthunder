...
$stamp = 'shell-player-' + (Get-Date -Format 'yyyyMMdd-HHmmss')
$logs = Join-Path $projectRoot "logs/022/$sha/$stamp"
$shots = Join-Path $projectRoot "docs/evidence/022/$sha/$stamp"
...
if (@($results | Where-Object { -not $_.passed }).Count -gt 0 -or $images.Count -ne 12) { exit 1 }
exit 0
