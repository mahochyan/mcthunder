...
# restore all touched files from HEAD first
...
# --- 1) explicit combat scope -------------------------------------------------------------------------
$ok = (Patch 'scripts/content/vehicle_catalog.gd' `
'const ENGINEERING_DIR := "res://configs/vehicles/engineering/"' @'
const ENGINEERING_DIR := "res://configs/vehicles/engineering/"
## WT-040-R1 (2026-09-17 ruling): the explicit combat scope. The curated historical roster keeps its contract and
## the engineering vehicles are a separate admitted set. Anything else is NOT a combat vehicle and must be refused
## rather than silently replaced by a training vehicle.
...
'@ 'catalog: combat scope statics') -and $ok

# --- 2) garage service: history stays history, engineering is explicit ---------------------------------
$ok = (Patch 'scripts/garage/garage_service.gd' `
...
'@ 'garage: explicit engineering load') -and $ok
...
'@ 'garage: has_vehicle scope') -and $ok
...
'	for id in VehicleCatalog.IDS:
	...
	for id in VehicleCatalog.IDS + VehicleCatalog.ENGINEERING_IDS:
		if has_vehicle(id): ids.append(id)
'@ 'garage: vehicle_ids scope') -and $ok

# --- 3) readiness ledger: see the engineering packets, and let the internal mode use preview-marked hulls
$ok = (Patch 'scripts/content/vehicle_readiness.gd' `
...
'@ 'readiness: engineering mode') -and $ok
...
'	var fallback := "training" if mode != "normal" else mode
	...
	for id in pool:
'@ 'readiness: fallback pool') -and $ok

# --- 4) team range: refuse instead of substituting, and never re-equip an engineering vehicle ----------
$ok = (Patch 'scripts/battle/team_range.gd' `
...
'@ 'team: own collision size') -and $ok

$ok = (Patch 'scripts/battle/team_range.gd' `
'	if selected_vehicle_id not in VehicleCatalog.IDS: return "player_tank"' @'
	# WT-040-R1 (2026-09-17 ruling): a vehicle that is neither curated history nor an explicitly admitted
	# engineering vehicle is REFUSED. It is never silently replaced by player_tank, and an admitted engineering
	# vehicle is used as itself - it is not rotated through the historical list and not re-equipped as training.
	if not VehicleCatalog.is_combat_vehicle(selected_vehicle_id):
		push_error("team match refused: selected vehicle is not an admitted combat vehicle: "+selected_vehicle_id)
		return ""
'@ 'team: refuse unknown selection') -and $ok
...
'@ 'team: no historical rotation for engineering') -and $ok
...
'	var fallback := VehicleReadiness.first_eligible([requested,selected_vehicle_id],"training",{},catalog)
	...
	return ""
'@ 'team: refuse instead of player_tank') -and $ok
...
'	if vehicle.definition.id not in VehicleCatalog.IDS: M4EngineeringProfile.apply(vehicle)' @'
	...
	# vehicle keeps its own admitted configuration, and anything that is not a combat vehicle is refused rather
	...
	elif not VehicleCatalog.is_combat_vehicle(combat_id):
		push_error("vehicle configuration refused: not an admitted combat vehicle: "+combat_id)
		return
'@ 'team: no training profile for engineering') -and $ok

$ok = (Patch 'scripts/battle/team_range.gd' `
'	if vehicle.definition.id not in VehicleCatalog.IDS:
		vehicle.gunner.shell.id = "team_ap120"' @'
	# WT-040-R1 (2026-09-17 ruling): the training round is for the training vehicle only; an engineering vehicle
	# keeps the round its own loadout installed.
	if combat_id == "player_tank":
		vehicle.gunner.shell.id = "team_ap120"' -and $ok

if (-not $ok) { Show "SOME PATCHES MISSED - aborting (files will be reported)"; }

# parse check every touched script
$g = 'E:\AIprogram\mcthunder\tools\godot\Godot_v4.7.2-stable_win64_console.exe'
$perrTotal = 0
foreach ($f in @('scripts/content/vehicle_catalog.gd','scripts/garage/garage_service.gd','scripts/battle/team_range.gd','scripts/content/vehicle_readiness.gd')) {
    $rel = 'res://' + $f.Replace('\','/')
    & $g --headless --path $c --check-only --script $rel *> (Join-Path $c 'logs\WT-040-R1\chkB.log') 2>&1 | Out-Null
    $n = @(Get-Content (Join-Path $c 'logs\WT-040-R1\chkB.log') | Select-String 'Parse Error|Compile Error').Count
    Show ("  parse " + $f + " errors=" + $n)
    if ($n -gt 0) { Get-Content (Join-Path $c 'logs\WT-040-R1\chkB.log') | Select-String 'Parse Error' | Select-Object -First 3 | ForEach-Object { Show ("      " + $_.Line.Trim()) }; $perrTotal += $n }
}
if ($perrTotal -gt 0) { Show "PARSE ERRORS - aborting"; exit 1 }

# dev-tree guard: the packaged-flow flank checks, then the garage and team suites
$lg = Join-Path $c 'logs\WT-040-R1\devtree-player-flow12.log'
Show "guard 1/3: player flow flank checks"
& $g --path $c --resolution 1280x720 -- --verify-player-flow *> $lg
$flank = @(Get-Content $lg | Select-String '^\[PASS\] normal keyboard and mouse session completes flank challenge|^\[PASS\] result backed by actual side penetration')
Show ("  flank pass lines=" + $flank.Count)
foreach ($suite in @('run_garage_checks','run_team_checks')) {
    $sl = Join-Path $c ('logs\WT-040-R1\B-guard-' + $suite + '.log')
    Show ("guard: " + $suite)
