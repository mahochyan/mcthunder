...
$r = Patch 'scripts/content/vehicle_catalog.gd' 'const ENGINEERING_DIR := "res://configs/vehicles/engineering/"' @'
...
$r = Patch 'scripts/garage/garage_service.gd' '	ready = catalog.load_all(definitions).ok' @'
...
$r = Patch 'scripts/garage/garage_service.gd' '	return ready and id in VehicleCatalog.IDS and catalog.packages.get(id,{}).get("ok",false) and definitions.vehicles.has(id)' @'
...
$r = Patch 'scripts/garage/garage_service.gd' '	for id in VehicleCatalog.IDS:
...
$r = Patch 'scripts/content/vehicle_readiness.gd' '	return "res://configs/vehicles/historical/"+id+".json"' @'
...
$r = Patch 'scripts/content/vehicle_readiness.gd' '	if mode not in ["training","normal"]: return {"ok":false,"code":"mode_restricted","reason":reason_text("mode_restricted")}
...
$r = Patch 'scripts/content/vehicle_readiness.gd' '	var fallback := "training" if mode != "normal" else mode
...
$r = Patch 'scripts/battle/team_range.gd' '	var size: Vector3 = defs.get_vehicle(type_id).drive_collision_size if type_id in VehicleCatalog.IDS else Vector3(2.85,1.68,5.45)' @'
...
$r = Patch 'scripts/battle/team_range.gd' '	if selected_vehicle_id not in VehicleCatalog.IDS: return "player_tank"' @'
	...
	if not VehicleCatalog.is_known_vehicle(selected_vehicle_id):
		...
		return ""
'@ 'team: training path kept, unknown refused'; $all = $r -and $all

$r = Patch 'scripts/battle/team_range.gd' '		requested = VehicleCatalog.IDS[(ids.find(id)%4+VehicleCatalog.IDS.find(selected_vehicle_id))%4]' @'
...
$r = Patch 'scripts/battle/team_range.gd' '	var fallback := VehicleReadiness.first_eligible([requested,selected_vehicle_id],"training",{},catalog)
...
$r = Patch 'scripts/battle/team_range.gd' '	if vehicle.definition.id not in VehicleCatalog.IDS: M4EngineeringProfile.apply(vehicle)' @'
...
$r = Patch 'scripts/battle/team_range.gd' '	if vehicle.definition.id not in VehicleCatalog.IDS:
...
$lg = Join-Path $c 'logs\WT-040-R1\devtree-player-flow14.log'
...
foreach ($suite in @('run_garage_checks','run_team_checks')) {
    $sl = Join-Path $c ('logs\WT-040-R1\B4-guard-' + $suite + '.log')
    Show ("guard: " + $suite)
...
$garageOk = ((Get-Content (Join-Path $c 'logs\WT-040-R1\B4-guard-run_garage_checks.log') -Raw) -match 'run_garage_checks: checks=\d+[^=]*passed=True')
$teamOk = ((Get-Content (Join-Path $c 'logs\WT-040-R1\B4-guard-run_team_checks.log') -Raw) -match 'run_team_checks: checks=\d+[^=]*passed=True')
