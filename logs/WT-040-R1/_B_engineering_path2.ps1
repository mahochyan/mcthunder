...
function Must([bool]$r, [string]$label) { if (-not $r) { $script:missed.Add($label) }; return $r }

$ok = (Must (Patch 'scripts/content/vehicle_catalog.gd' `
...
'@ 'catalog: combat scope statics') 'catalog-statics') -and $ok

$ok = (Must (Patch 'scripts/garage/garage_service.gd' `
...
'@ 'garage: explicit engineering load') 'garage-load') -and $ok
...
'@ 'garage: has_vehicle scope') 'garage-has') -and $ok
...
'	for id in VehicleCatalog.IDS:
	...
	for id in VehicleCatalog.IDS + VehicleCatalog.ENGINEERING_IDS:
		if has_vehicle(id): ids.append(id)
'@ 'garage: vehicle_ids scope') 'garage-ids') -and $ok

$ok = (Must (Patch 'scripts/content/vehicle_readiness.gd' `
'	return "res://configs/vehicles/historical/"+id+".json"' @'
	...
	return "res://configs/vehicles/historical/"+id+".json"
'@ 'readiness: packet path') 'readiness-path') -and $ok
...
'@ 'readiness: engineering mode') 'readiness-mode') -and $ok
...
'	var fallback := "training" if mode != "normal" else mode
	...
	for id in pool:
'@ 'readiness: fallback pool') 'readiness-pool') -and $ok

$ok = (Must (Patch 'scripts/battle/team_range.gd' `
...
'@ 'team: own collision size') 'team-size') -and $ok
...
'	if selected_vehicle_id not in VehicleCatalog.IDS: return "player_tank"' @'
	...
	# vehicle is used as itself - not rotated through the historical list and not re-equipped as training.
	if not VehicleCatalog.is_combat_vehicle(selected_vehicle_id):
		...
		return ""
'@ 'team: refuse unknown selection') 'team-refuse') -and $ok
...
'@ 'team: no historical rotation for engineering') 'team-rotation') -and $ok
...
'	var fallback := VehicleReadiness.first_eligible([requested,selected_vehicle_id],"training",{},catalog)
	...
	return ""
'@ 'team: refuse instead of player_tank') 'team-refuse2') -and $ok
...
'	if vehicle.definition.id not in VehicleCatalog.IDS: M4EngineeringProfile.apply(vehicle)' @'
	...
	if combat_id == "player_tank":
		...
		return
'@ 'team: no training profile for engineering') 'team-profile') -and $ok
...
'	if vehicle.definition.id not in VehicleCatalog.IDS:
	...
	if combat_id == "player_tank":
		...
		vehicle.gunner.shell.armor_policy = "resolve"
		vehicle.gunner.shell.penetration_curve = PackedVector2Array([Vector2(0,120),Vector2(200,120)])
'@ 'team: training round only') 'team-round') -and $ok

if ($missed.Count -gt 0) { Show ("PATCHES MISSED: " + ($missed -join ', ')); Show "aborting without running anything"; exit 1 }
...
$garageOk = ((Get-Content (Join-Path $c 'logs\WT-040-R1\B-guard-run_garage_checks.log') -Raw) -match 'run_garage_checks: checks=\d+[^=]*passed=True')
