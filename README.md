...
原创体素／低模方块视觉的坦克战斗原型。总体目标是尽可能还原战争雷霆式陆战玩法；完整路线见 [开发计划](docs/handoff/DEVELOPMENT_007_036.md)。当前开发版已实现装甲结算、内部损伤、弹药账本、火灾与维修/乘员替补。真实路径回放已可用；AI、车库与完整比赛仍按依赖推进；计划不是已完成内容。
...
## 自动验证
`pwsh -File tests/run_suite_checks.ps1 -Order 010`：固定引擎导入 + 基础/布局/查询/弹道/装甲/损伤/恢复/回放八套检查。
