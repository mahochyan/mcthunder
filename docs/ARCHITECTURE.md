...
## 025 低模贴图、特殊建筑与真实残骸
VehicleAtlas按实际ArmorPatch顶点和三角形生成外皮，从车型manifest读取同顶点UV；Blender导出外饰保持hull/turret/barrel/recoil轴系。HistoricalTrackMotion仅读取实际位移和转向，独立滚动左右履带纹理。Skin和Cosmetic的完整可见模型限制1100三角形；分配的烟火另计实体预算。可移植模型流程见TANK_MODEL_WORKFLOW.md。
ProjectileManager的最近世界接触向DestructibleSection提交唯一弹丸身份，破坏更新碰撞后发出表现事件；击毁该段的当前弹丸仍在世界接触处结束。StructureCollapse与StructureDust有界、无伤害。WreckTurretMotion挂接实际炮塔节点，残骸查询、炮塔落地阻挡和显示共同读取其位置；RecoveryVisuals只观察既有死因，使用双面烧焦材质并在重置时恢复原材质。没有视觉专用伤害或原位隐形炮塔。
PlayerController释放门同时检查当前帧、下一帧边沿和按住状态，避免菜单同帧按下/松开延迟成为开火边沿。清除释放门的那一帧不提交射击；真实输入回归保留自然倒计时和装填。

026：实际ProjectileManager拥有CombatFeedback，只读发射、接触、死亡与车辆连续状态。32音源/12粒子组固定池，按事件身份去重；视觉与声音不写伤害或计分。暂停停音，终局停循环并允许最后事件声结束，离场释放。实现预算见AUDIO_FX_026.md，验证见DELIVERY_026.md。
