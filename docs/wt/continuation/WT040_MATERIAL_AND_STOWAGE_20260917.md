...
`7f89c941` 原地图、种子 44001、4v4、16v16 布局、原比赛参数与判据，独占无窗口实际运行：`modern-match-restored-policies.log/json`，**8 项、1 失败，真实 exit 1**。306.733 秒自然结束，票数 0∶214；两队分别到达 C/A 与 B/A，健康且无阻挡、有路的异常静止峰值 5 秒，通行门通过。仅 A4、B/B2/B3/B4 五个槽位开火，低于原六槽位参与要求，完整 AI 检查保持 FAIL。
