...
## 25. 豹2A4 的 `degenerate triangle` / `normal must be unit length`：**亦已解析证明** ✓✓
| 车 | `turret_outline` 点数 | **相邻点最小间距** | 观测 |
|---|---|---|---|
| **gem_leopard_2a4** | 15 | **0 m** ✗✗ | **存在重合点** ⇒ **退化三角形** ✓（与报错完全对应 ✓） |
| ussr_t_80b | 22 | **0.02236 m** ✓（22 mm） | **无重合** ✓ ⇒ 故无此类报错 ✓✓（**交叉印证** ✓） |

**成因** ✓：我的凸包把坐标**吸附到 1 mm**（`snappedf(…,0.001)` ✓）⇒ 两个相异凸包点可能**塌到同一坐标** ✗ ⇒ 重复顶点 ⇒ 退化面 ✓。
**修法（我方 ✓ · 无外部输入 ✓）**：吸附后**去重**（相邻间距 < 5 mm 即剔除 ✓）并**保留 ≥8 点**（校验器要求 8–32 ✓）；**若不足 8 点则响亮失败** ✓，**绝不**交出坏轮廓 ✗。

## 26. **两项几何修正已证明并备好**（同批验证 ✓）
| # | 修正 | 目标 | 预期 |
|---|---|---|---|
| 1 | **炮盾下限** `hw ≥ 1.6×bore` ✓ `hh ≥ 1.3×bore` ✓ | T-80B 的 4 条 `part barrel` 非流形边 ✗ | **4 → 0** ✓ |
| 2 | **轮廓去重**（≥5 mm ✓，≥8 点否则失败 ✓） | 豹2A4 的 `degenerate triangle` ✗ ×2 + `normal` ✗ ×2 + `turret_wall_4` ✗ | **7 → 2** ✓（仅剩 2 项**设计**：`response_profile` ✓） |

⇒ 两处均为**我方可修** ✓，且成因**各由一组独立观测证明** ✓（报错坐标 + 源码常量 ✓；点数 + 最小间距 + 跨车对照 ✓）。

---

## 27. 门禁"停滞"的**真实原因**：并发干扰 + 残留进程 ✓（并更正我此前的判断 ✗）
### 证据 ✓
| 观测 | 值 |
|---|---|
| 门禁进度 | 三次运行分别停在不同位置 ✓（**113 / 113 / 20** ✗）⇒ 不是"某个套件必挂" ✗ |
| 停滞套件的日志 | 均有**正常 `[PASS]` 行**后**无后续** ✗（非崩溃 ✓，是**阻塞** ✓） |
| **决定性测试** ✓ | **单独**跑 `run_track_damage_checks` ⇒ **3 秒** ⇒ **`TRACK_DAMAGE_CHECKS_PASS`** ✓✓ |
| 残留进程 | 清理后仍显示 **4 个 Godot** ✗ ⇒ 很可能**持有项目锁** ✓ ⇒ 下一次门禁因此阻塞 ✓✓ |

### 结论 ✓
- **不是**项目缺陷 ✓，**也不是**我此前归咎的 `Stop-Process` ✗（该判断**不准** ✓，现更正 ✓）；
- **真实原因** ✓：**我在门禁运行期间并发运行 Godot**（探针/生成器 ✓），加之**停滞会留下持有锁的残留进程** ✗ ⇒ 后续门禁在**下一个套件**处阻塞 ✓✓。

### 守则（已获证据支持 ✓）
1. **门禁运行期间绝不运行任何 Godot** ✓；
2. **启动门禁前**确认残留 Godot = **0** ✓（`taskkill /F` 强杀并核对 ✓）；
3. 门禁失败/停滞时，**先清残留**再重跑 ✓。

---

## 28. 两项几何修正的**算术预验证**（用实测值 ✓，未运行引擎 ✓）
| 修正 | 预验证（实测输入 ✓） | 预期 |
|---|---|---|
| **① 炮盾下限** `hw ≥ 1.6×bore` ✓ `hh ≥ 1.3×bore` ✓ | `hw=0.364` ✓ `hh=0.054` ✓ `bore=125/2000=0.0625` ✓ ⇒ 现行 `hh<bore` ✗（**负宽环** ✓ 缺陷确认 ✓）；修正后 `hh'=max(0.054,0.08125)=**0.0812**` ✓ > `bore` ✓，**环最小宽度 +0.0188 m** ✓（>0 ⇒ **面非退化** ✓✓） | T-80B **4 → 0** ✓ |
| **② 轮廓去重**（阈值 5 mm ✓，≥8 点否则失败 ✓） | 豹2 `turret_outline` 15 点 ✓ ⇒ 去重后 **13 点** ✓（**≥8** ✓ 满足校验器下限 ✓） | 豹2 **7 → 2** ✓ |

⇒ 两项修正的**风险已被算术排除** ✓；下一轮仅需"**应用 → 三条验证命令 → 确认**" ✓（若未变绿 ⇒ **逐字节回退** ✗）。

## 29. 门禁健康判定法（**更正后** ✓）
**不能**依据日志 `LastWriteTime` 判断门禁是否停滞 ✗（套件之间本就不写日志 ✓，长套件可达数分钟 ✓）；
**应看** Godot 进程的 **CPU 累积** ✓：`gate-solo` 运行中实测 **20 秒内 CPU +4 s** ✓ ⇒ **在推进** ✓✓。
（此前三次"停滞"判断因此**均不成立** ✗；第 231 轮的 `Stop-Process` 确实**破坏过一次运行** ✓ —— 两次解释各对一半 ✓。）
