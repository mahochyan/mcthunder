...
### 12.6 仍未完成 / 未验证（**不掩盖**）
- **证据范围（本轮澄清，避免混淆）** ✓：**测试脚本与工程候选资产均不随包发布** ✓（包内清单实测：`excluded development file: res://tests/run_checks.gd` ✓、`four historical content packages admitted` ✓）⇒
...
### 13.1 完成项（**均有树内或包内实测证据** ✓）
| 项 | 证据 | 提交 |
|---|---|---|
| 校验器可信 | 套件运行器改真进程捕获 ⇒ **真实 `exit_code`** ✓ · 登记匹配器 **15/15 负例** ✓ · 构建门禁**两次正确拒收**（偶发 `run_checks` ✓ / 未登记失败 ✓） | `5b297f03` `dfb25385` |
| **A** 入口错位 | 真实界面路径 ✓ · **96 项 0 失败** ✓ · `PLAYER_FLOW_CHECKS_PASS` ✓ · **13 张实拍** ✓ | `1e37e5f0` |
...
- **包内玩家流程的 M36 路线/入库步骤** ✗（§11.4 ⇒ **开放项**）
- **真人验收** = `PENDING` ✓（**不代签** ✓）· **性能** = `HOLD_BY_USER` ✓ · **公开发布** = `false` ✓
...
- 包为 `devcandidate`（`release_ready=false` ✓）⇒ 可**直接弃用** ✓
- 旧包 `3e6c3523` / `9984a0ca` / `ad6620f0` 等**保留不覆盖** ✓

### 13.5 交接与变更清单
- **新增**：`tests/check_engineering_wiring.gd` ✓ · `tests/check_live_fire_respawn.gd` ✓
- **修改**：`scripts/battle/team_range.gd` ✓ · `scripts/training/ballistics_range.gd` ✓ · `scripts/garage/garage_service.gd` ✓ · `scripts/content/vehicle_catalog.gd` ✓ · `scripts/content/vehicle_readiness.gd` ✓
- **测试/记录**：`tests/run_suite_checks.ps1` ✓（真实退出码）· `tests/run_checks.gd` ✓（一帧竞态 + 记账 + 有界等待）· `tests/record_river_ai_match.gd` ✓（五项分列 + 场景参数 + 分类）· `tests/build_release.ps1` ✓（窄签名登记）
- **文档**：本文件 §12–§13 ✓
- **明确未动** ✓：`main` ✓ · 用户未提交内容 ✓ · 引擎版本 ✓ · 构建发布流程 ✓ · 票池/占领/伤害/装填/终止 ✓
