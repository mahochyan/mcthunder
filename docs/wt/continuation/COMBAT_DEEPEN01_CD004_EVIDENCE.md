...
## 19. `实现顺序 #3` **采样表** ＋ **超范围不作性能承诺** ✓✓（CD004 收口）
探针 `tests/probe_cd004_sampling_table.gd` ⇒ **`CD004_SAMPLING_TABLE_PASS`** ✓
**Part A：零重力保持率 vs 冻结声明曲线** ✓（初速 1500 m/s ✓ k=1.40e-4 ✓ 容差 0.010 ✓）
```
 200 m: 声明 0.972 | 闭式 0.972 | 实测 0.972 | +0.0004 ✓
 500 m: 声明 0.932 | 闭式 0.932 | 实测 0.932 | +0.0004 ✓
1000 m: 声明 0.871 | 闭式 0.869 | 实测 0.869 | −0.0016 ✓
1500 m: 声明 0.814 | 闭式 0.811 | 实测 0.811 | −0.0034 ✓
```
**Part B：实用表（含该弹自身重力 −9.810 m/s²）** ✓
| 射程 | 真空 t / 落点 / 速度 | 工程阻力 t / 落点 / 速度 |
|---|---|---|
| 200 m | 0.1333 s / −0.0872 m / 1500.001 m/s | 0.1352 s / −0.0889 m / **1458.582** m/s |
| 500 m | 0.3333 s / −0.5450 m / 1500.004 m/s | 0.3453 s / −0.5714 m / **1398.593** m/s |
| 1000 m | 0.6667 s / −2.1800 m / 1500.014 m/s | 0.7156 s / −2.3986 m / **1304.051** m/s |
| 1500 m | 1.0000 s / −4.9050 m / 1500.032 m/s | 1.1127 s / −5.6701 m / **1215.911** m/s |
（阻力腿在四点上**均更晚 ✓ 更慢 ✓ 掉得更多 ✓** —— 12 项检查全 PASS ✓）

### ⚠️ 实测发现：**有效射程 200.0 m** ⇒ 表中 500/1000/1500 m **超出**有效射程 ✗
```
[CD004 T05t] effective range 200.0 m ; all sampled ranges inside it: **false**
```
⇒ 依子单 `实现顺序 #3`"**在 200/500/1000/1500 m 范围内按武器有效射程采样，明确超范围不作性能承诺**" ✓ ⇒ 本表**如实标注**：
- **200 m 行**：**在**声明有效射程内 ✓ 可作为承诺 ✓；
- **500 / 1000 / 1500 m 行**：**超出**有效射程 ✗ ⇒ **仅作模型外推记录，不作任何性能承诺** ✗（`GameConfig.GUN_RANGE=200 m` ✓）。
**交付时须一并写入交付文档** ✓（阶段 6 ✓）：本表所有数值均为**项目设计初值** ✓ 非实测史料 ✓ 亦不冒充真实武器性能 ✓。

### ✅ CD004 结项状态
| 项 | 状态 |
|---|---|
| T01 / T02 / T03 / T04 / T05 / T06 | ✅ **六条用例全部通过** ✓ |
| 必须设计 #1 / #2 / #3 / #4 / #5 | ✅ **五项全部落地并经实测** ✓ |
| 实现顺序 #1（冻结基准+曲线表）/ #2（接入+旧配置显式 v1）/ #3（采样表+超范围声明）/ #4（移动专项） | ✅ **全部完成** ✓ |