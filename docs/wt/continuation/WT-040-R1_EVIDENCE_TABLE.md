...
## 29. 🎯 **五个导出预设逐个读毕** ✓ —— 只读排除法**穷尽** ✓
| 项 | 结论 |
|---|---|
| 五个预设 **全部** `export_filter="all_resources"` ✓ | `.tres` **属资源** ⇒ **会被导出** ✓ |
| 排除表（`docs/*` `logs/*` `backups/*` `builds/*` `tools/*` `order/*` `authoring/*` `deliverables/*` `dev_tools/*` `tests/*` `assets/reference_data/*` + `*.md` `*.ps1` `*.py` `*.zip` `*.bat` `*.blend`） | **不含 `configs/`** ✓ |
| `configs/` 内容 ✓ | **12 个 `.tres`** ✓ + **8 个 `.json`** ✓ ⇒ **均在范围内** ✓ |
| 产出该包的 `[preset.4] "Windows Client"` ✓ | 过滤同上 ✓，**未排除 configs** ✓ |

⇒ **排除法穷尽** ✓：**不是**过滤 ✗ · **不是**加载方式 ✗ · **不是**代码 ✗ · **不是**已跟踪资源 ✗ ⇒
**病因必在导出过程本身** ✓ ⇒ **纯只读分析到此为止**（**不编第四个假设** ✗）。

**判定计划** ✓（见决策表 ④c ✓）：修一行 ⇒ 新构建 ⇒ 跑 `run_player_flow_checks` ⇒ **二值互斥** ✓：
仍缺档案 ⇒ **导出过程缺陷确认** ✓；通过 ⇒ **旧包个别异常** ✓，④b **即消** ✓。

---

## 30. ✅ **方法标定**：对导出包做纯字符串扫描**零判别力** ✗✓（取证线程就此收束 ✓）

### 30.1 标定做法 ✓（搜"必然在包内"的路径 ✓）

| 搜索项（工程内**确实存在** ✓） | 包内明文出现 |
|---|---|
| `scenes/tank.tscn`（游戏**必然加载** ✓） | **未命中** ✗ |
| `scenes/app.tscn` · `scenes/main.tscn` | **未命中** ✗ |
| `scripts/vehicle_actor.gd` · `scripts/defs/vehicle_defs.gd` | **未命中** ✗ |
| `configs/player_tank_vehicle.tres` · `_weapon.tres` | **未命中** ✗ |
| **命中率** | **1 / 11** ✗（唯一命中 `ui_` ✗ —— **通用子串**，非路径 ✓） |
| `configs/` · `tank.tscn` 明文次数 | **0** ✗ · **0** ✗ |

### 30.2 结论 ✓✓

**Godot 导出包的路径表被压缩/编码** ⇒ **纯字符串扫描对"包内是否含某文件"零判别力** ✗✗
⇒ **第 309 轮那批"未命中"不构成任何证据** ✓（当时已标注其不决定性 ✓，**现获证明** ✓）。

### 30.3 取证线程收束 ✓

**④b 只能由"新构建"（或专用 pck 读取器）判定** ✓ —— 与决策表 **④c** 的**二值计划一致** ✓：
授权 ⇒ 修一行 ⇒ 新构建 ⇒ 跑 `run_player_flow_checks.ps1` ⇒
**仍缺档案** ✗ ⇒ 导出过程缺陷**确认** ✓ ／ **通过** ✓ ⇒ 旧包个别异常，**④b 即消** ✓。

### 30.4 本会话**方法论清单**（同族教训 ✓，全部有据 ✓）

1. **判据本身可能失效而不自知** ✓（`Latin1` 为 null ✗ · 只数 `[FAIL]` 不看退出码 ✗ · 假套件名报绿 ✗）；
2. **"看起来卡住/失败"须看权威信号** ✓（套件完成数 + 引擎本体 CPU ✓；**非**总日志时间戳 ✗、**非** console 包装器 CPU ✗）；
3. **空摘要 ≠ 什么都没发生** ✓（`combat_summary` 为玩家口径 ✗）；
4. **方法须先标定** ✓（控制组 + 长度守卫 ✓；本例证明"未命中"无意义 ✗）；
5. **结论以命令输出为准** ✓（活文档不写死漂移数字 ✓）。
