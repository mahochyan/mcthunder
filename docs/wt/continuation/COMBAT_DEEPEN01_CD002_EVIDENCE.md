...
### 2.5 板件轮廓 vs 交付模型网格：**已测，逐处登记为未验** ✓（数值已更正 ✗→✓）
...
**坐标系更正（我自己的错 ✗）**：第一版把网格盒算在**角色根的父坐标系**里 ✗（炮塔被 `turret_origin=[0,1.447,0]` 抬起 ⇒ 假"下方超差 1.38 m" ✗），而板件顶点是**部件自身系** ✓ ⇒ 现改为**从根的子节点以单位变换累计** ✓，得到部件自身系 ✓（炮塔 Y∈[−0.030, 0.963] ✓ 正与 `turret_bottom=−0.031` / `turret_top=0.585` 吻合 ✓）。判定＝**包含性**（板件是部件的子区域 ✓ 不得越出所属部件模型盒 ✓，**排除子角色**以免车体盒被炮塔/火炮撑大 ✓），容差＝**50 mm 或 2%** 按轴取大 ✓。

| 车 | 修正后的实测超差（关键区 ✓） | 其余 |
|---|---|---|
| `ussr_t_80b` | 车体后部 5 区 **210 mm**（`hull_sides_rear` / `hull_sides_lower_rear` / `hull_rear_upper` / `hull_rear_lower` / `hull_roof_rear` ✓）· 炮塔 `front` **99.7** / `sides` **332.1** / `rear` **575.7** / `roof` **233.7** mm ✓ · 炮盾 **663.1 mm** ✓ | 前/侧/顶/底多数区 **within（outside 0.0 mm）** ✓ |
| `germ_leopard_2a4` | **仅炮盾 60.8 mm** ✓ | **全部 12 车体区 + 炮塔 4 区 within** ✓ |

**登记（用例原文允许"超差…或列未验" ✓）** ⇒ 上表 11 处**逐处登记为未验** ✓，实测值写入探针常量 `MESH_UNVERIFIED_ITEMS` ✓（T-80B 10 项 ✓ 豹2 1 项 ✓），并写进导出 JSON 的 `mesh_overlay`（含 `outside_mm` / `inset_mm` / `limits_mm` / `within` / `registered_unverified` / `unlisted` / `drifted` ✓）。
**探针断言（不放宽 ✗）**：① 存在**任何未登记**的超差关键区 ⇒ 红 ✓；② 已登记项的实测值**漂移 > 1 mm** ⇒ 红 ✓。当前两者均为空 ✓（`unlisted=[] · drifted=[]` ✓）。

**各处待判定的问题**（下一轮在**隔离 authoring 目录**内判并修 ✓ 不碰你在制作的 GLB ✓）：
1. 两车**炮盾**越出 `gun` 角色 663.1 / 60.8 mm ⇒ 判定"**模型炮盾网格偏小**"还是"**`mantlet_half_*` 估计偏大**" ✓；
2. T-80B **炮塔** 4 区越出 99.7–575.7 mm ⇒ 判定"**模型炮塔轮廓偏小**"还是"**`turret_outline` 估计偏大**" ✓；
3. T-80B **车体后部** 5 区越出 **210 mm** ⇒ 判定"**模型尾板偏短**"还是"**`hull_rings` 后段偏长**" ✓。
三者均为**项目内部一致性**问题（交付模型 ↔ 窄相位几何 ✓）**与战雷对照无关** ✓ ⇒ 标 `NOT_COMPARED` ✓；每处的实际误差已按轴记录 ✓。

**逐处判定（结论 ✓）**：生成器**忠实使用包内授权数据** ✓ ⇒ **无生成侧空间错误** ✗ ⇒ 11 项**全部**是"**包内授权几何 vs 交付低多边形模型覆盖**"的分歧 ✓ ⇒ **登记未验即为正确处置** ✓（不把授权几何缩小去迁就低模 ✗ —— 那正是纪律禁止的"无对照改数据" ✗）。

| 项 | 布局板件实测 | 交付模型 | **布局来源包（`authoring/reference_data/…` ✓）的授权参数** | 判定 |
|---|---|---|---|---|
| T-80B 车体后部 ×5 | Z 到 **3500 mm** | Z 到 3290 ✗ | `hull_rings` Z max = **3.5 m**（=3500 ✓） | 布局**忠实** ✓ ⇒ 模型覆盖不足 ✗ |
| T-80B 炮塔 `sides` | X **±1450** | ±1282/1285 ✗ | `turret_outline` X = **±1.45** ✓ | 同上 ✓ |
| T-80B 炮塔 `rear`/`roof`/`front` | Z **1900 / 1558 / −1400** | 1324 / 1324 / −1300 ✗ | `turret_outline` Z = **1.9** ✓；`turret_bottom/top` = **0.04/0.75** ✓ | 同上 ✓ |
| 两车**炮盾** | `[−500,−270,−100]..[500,270,−100]` | gun 角色 ±168（T-80B）/ ±443–496（豹2）✗ | `mantlet_half_width/height` = **0.5/0.27** ✓ | 同上 ✓ |

**更正记录（我自己的三处同源错误 ✗）**：① 从角色根自身累计网格 ⇒ 父坐标系 ✗（假"炮塔下方 1.38 m"）；② 取夹具 packet 的模型路径 ✗（已被 TEST ONLY 模型改写）；③ 用**生产配置**的参数去比**参考包**构建的布局 ✗（假"超出自己参数 345 mm"）。三处均由实测暴露并更正 ✓；更正后的判定如上表 ✓ 与原判定**相反** ✓（这正是"先测再判"的价值 ✓）。
