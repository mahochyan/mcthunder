...
**逐处判定（本轮 ✓ 依据：逐轴实测 vs **包内授权参数** ✓ 不是猜 ✓）**

| 项 | 布局板件实测 | 交付模型 | 包内授权参数 | 判定 |
|---|---|---|---|---|
| T-80B 车体后部 ×5 | Z 到 **3500 mm** | Z 到 3290 ✗ | `hull_rings` 最大 **Z=3155** ✓ | **超出自己授权参数 345 mm** ✗ ⇒ **疑似真实内部不一致**（待读环构造确认 ✓） |
| T-80B 炮塔 `sides` | X **±1450** | ±1282 / 1285 ✗ | `turret_outline` X ≈ **±1282** ✓ | 超出授权参数 ⇒ 待确认 `face()/annulus()` 是否加壁厚 ✗ |
| T-80B 炮塔 `rear` / `roof` / `front` | Z **1900 / 1558 / −1400** | 1324 / 1324 / −1300 ✗ | outline Z ≈ **±1278** ✓ | 同上 ✗ |
| 两车**炮盾** | `plate=[−500,−270,−100]..[500,270,−100]` | gun 角色 ±168（T-80B）/ ±443–496（豹2）✗ | **恰等于包内 `declared_openings.shield_perimeter`**（±0.5, ±0.27, −0.1 ✓） | 生成器**忠实使用授权数据** ✓ ⇒ **模型覆盖 vs 包内授权**的分歧 ✗（**非布局缺陷** ✓）⇒ 登记未验 ✓ |

⇒ 即：**不能一刀切** ✓ —— 炮盾一类是"**模型未网格化到包内声明的开口范围**" ✓（包内数据是项目授权源 ✓ 模型是 low-poly ✓），车体后部/炮塔一类**疑似生成侧超出授权参数** ✗，后者才是"**已确认的空间错误**"应当修复 ✓。
