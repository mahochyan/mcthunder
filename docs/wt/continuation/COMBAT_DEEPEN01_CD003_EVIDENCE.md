...
### 3.6 `CD03-T06` 旋转（3C）：**已闭合** ✓（第三次尝试成功 ✓ 前两次被实测否定 ✓ 留档）
**最终实现** ✓（三处同做，缺一不可 ✓）：
1. `translation_sweep.gd::part_transform` **插值基座**（`slerp` ✓）+ 新增 `rotation_angle()` / `local_sub_segment()` ✓；
2. `shot_query_service.gd` 内**按声明角度步长 `ROTATION_STEP_RAD = 0.02 rad` 细分子段** ✓ ⇒ 每子段**单一坐标系**的局部弦 ✓ + **逐子段的射线偏移换算** ✓（即 3B 的接触帧换算 ✓ 在旋转下才有意义 ✓）+ 子段参数**映射回全局 `t`** ✓（保持单一时间线 ✓）；
3. **部件级粗筛对旋转件跳过** ✓（整步局部线段在基座旋转后**混两个系** ✗ ⇒ 曾把整部件误标 missed ⇒ 板件循环根本不跑 ✗）⇒ 旋转件改为**在板件循环内逐子段粗筛** ✓。

**实测（闭合 ✓）**：
```
rotating 0→45°: query fraction = 0.536451  vs  analytic 0.537424   ⇒ delta = 0.000973 ✓✓（原 0.045910 ✗，改善 ~47×）
                local_x = −0.004908 ≤ 声明弦界 0.013098 m（= 局部半径 0.546 m × 0.02 rad × 1.2 ✓ 界由几何给出 ✓）
frozen  0→0°  : 0.500000 = analytic ✓（静态路径不变 ✓）
探针自有独立细分（不依赖引擎助手 ✓）同样得 0.536451 ✓ ⇒ **查询与独立参考逐位一致** ✓
```
回归 ✓：**11 套件 558 PASS / 0 FAIL** ✓（含 `ARMOR 81 · AI_INTERCEPT 106 · AUTHORITY 19` ✓）+ 探针 `CD003_GAP_BASELINE_PASS` ✓。

**被实测否定的两次尝试**（历史 ✓ 有价值的负结果 ✓）：
1. **只插值基座** ✗ ⇒ 旋转件**完全不命中** ✓（`local_segment` 两端点在不同系 ✓）；
2. **基座插值 + 子段细分但未改部件级粗筛** ✗ ⇒ **仍完全不命中** ✓（`fraction = −1.000000` ✓ 比原状更差 ✗）。
⇒ 第 2 次失败后**先仪器化** ✓（探针自建独立插值 + 同一三角面例程 ✓）⇒ 测出**弦几何正确**（40 段时 0.536451 ✓ 与解析一致 ✓）⇒ 从而把故障**锁定到查询管线** ✓ ⇒ 第三次据此修复并成功 ✓✓。**教训记档**：同类"保守粗筛未随运动/截面放宽"的缺陷在本单已出现**两次**（3A 的部件粗筛 ✓、3C 的部件粗筛 ✓）⇒ 凡引入新的窄相位维度 ✓ 必须**同步审查所有粗筛** ✓。
...
### 3.7 截面扩展到**模块/乘员**（`必须设计 #4` ✓ 已落）
`_collect_boxes`（模块与乘员共用 ✓）现接收 `section_radius`/`section_offsets` ✓，在**同一 box-local 坐标系**内逐环射线测试 ✓：
- **中心射线优先** ✓ ⇒ 一个部件在一个 episode 仍是**一个** interval ✓（不重复计 ✓）；
- 逐收集器**各自声明射线预算** ✓（`RAY_BUDGET` ✓ 耗尽 ⇒ `ray_budget_exhausted` + `complete=false` ✓）；
- **粗筛审查** ✓（按上一轮教训 ✓）：该函数内**无粗筛** ✓ ⇒ 无需同步放宽 ✓（教训已应用 ✓）。

**实测（独立判据 ✓）**：一个 5 cm 的模块箱放在中心线**旁边**（局部 y=60 mm ✓）：
```
无截面：centre-only intervals = 0 ✓（中心线错过它 ✓）
有 60 mm 截面：intervals = 1 ✓✓（该线命中了弹丸宽度足以覆盖的模块 ✓）
```
回归 ✓：**13 套件 685 PASS / 0 FAIL** ✓

**仍未做** ✗：**世界遮挡**（`world_stop` 由调用方提供的单条世界接触 ✓）尚未按截面采样 ✓ ⇒ 记为下一步 ✓；**后效快照时刻** ✓ 亦待按 `必须设计 #4` 统一 ✓。

### 3.8 世界遮挡按截面采样（`必须设计 #4` ✓ 已落）
`WorldQueryAdapter.query_world_stop` 增加**可选** `section_radius`/`section_rays`（**保持所有既有调用者不变** ✓）⇒ 有截面时取**有界环射线**的**最近命中** ✓ 并受**自身声明的预算**限制 ✓；无截面时**逐字保持**原单射线行为 ✓。
**实测（独立判据 ✓）**：一条 50 mm 厚的薄墙放在中心线旁（抬高到 y=500 mm 以避开地面 ✓）：
```
无截面：hit=false ✓（中心射线错过 ✓）
有 60 mm 截面：hit=true ✓✓（弹丸宽度足以刮到该墙 ✓）
```
⇒ 至此子单 `必须设计 #4` 所列的落点链：**装甲 ✓ 模块 ✓ 乘员 ✓ 世界遮挡 ✓** 均消费同一声明截面 ✓（**后效快照时刻**仍待统一 ✗）。

### 3.10 后效快照时刻：**已量化开放** ✗（不盲改 ✓ 修法待其判据就位）
**机制** ✓：`ShellEffectPolicy.on_inside_path(st, snapshots, dir, maximum)` 经 `target_snapshot` 使用快照的 **`part_world_transforms` = 步末坐标系** ✗，而弹丸在**查询起点** ✓、接触在步**内**某 fraction ✓ ⇒ **混合时刻** ✗ —— 正是 `必须设计 #4` 所指"装甲、模块、乘员、世界遮挡**和后效**统一采用接触时刻"中的**最后一项** ✗。
**实测（独立判据 ✓）**：同一个真实车辆布局、同一个点，只在**两种坐标系**下问"该点是否在封闭腔体内"：
```
point=(0.0, 0.9, 0.025)
inside(步末坐标系, 目标位移 +4.00 m) = false ✗   ← on_inside_path 当前使用的即此帧
inside(接触时刻,   目标位移 +2.16 m) = true  ✓
```
⇒ **谓词随坐标系改变** ✓ ⇒ 移动目标下后效会以**目标尚未到达的位置**求值 ✗ ⇒ 该差异**可测** ✓（不是无操作 ✓）。
另记一次夹具自伤 ✓：首版用**零厚度平板**问 `inside` ✗ ⇒ 恒为 false（该谓词判**封闭腔体** ✓）⇒ 改用真实布局与车体中心点后判据成立 ✓。

### 3.11 `run_checks` 超时：**基线对照已证与本次改动无关** ✓
```
基线（1e8a23bb，世界遮挡改动之前，先 --import 建缓存 ✓）：exit=2 · PASS=177 · FAIL=0 · WATCHDOG=1 · 无 marker
本次树（含世界遮挡改动）                               ：exit=2 · PASS=177 · FAIL=0 · WATCHDOG=1 · 无 marker
```
⇒ **逐项一致** ✓ ⇒ 该套件在本无头工作树内的 **90 秒看门狗超时是基线既有行为** ✓ **与本次改动无关** ✓（对照而非假设 ✓）；仍**不记为通过** ✓ ⇒ `NOT_RUN（环境性超时）` ✓。
另记 ✓：第一次基线尝试**无效** ✗（新工作树无 `.godot` 导入缓存 ⇒ 套件连启动都没启动 ✓ `exit=1 · 0 项` ✗）⇒ 补 `--import` 后才成为有效对照 ✓。

### 3.12 明确未完成（`拒收条件 4` ✓ 不得只做 3A 就宣称整单完成 ✓）
我在上一提交信息里把 `run_checks`（主检查集）列入"全绿" ✗ —— **实际是 `[WATCHDOG] 90s 超时，强制退出`** ✓（`exit=2` ✓ **无 PASS/FAIL 标记** ✓；超时前已 **177 项 PASS、0 FAIL** ✓ ⇒ **不是断言失败** ✓）。
⇒ 该套件在本无头工作树内**未跑完** ✓ ⇒ 记为 **NOT_RUN（环境性超时）** ✗ 而非"通过" ✓；其与本次改动的关系**尚未核实** ✓ ⇒ 下一轮用**基线对照**核实（在 `main` 基线工作树内同样跑一次 ✓ 或对比未改前的同一命令 ✓），**不假设它无关** ✓。
