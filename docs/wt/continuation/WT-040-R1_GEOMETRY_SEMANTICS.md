...
## 6. 第三轮：自适应分带 ✓ + **薄片拒收** ✓（工具学会区分"测到了"与"像测到了" ✓）
| 车辆 | 字段 | hull_rings | 依据 |
|---|---|---|---|
| **ussr_t_80b** | **15** ✓ | **产出** ✓（ring 跨度 8.3 / 9.7 / 9.1 m ✓，均 > 整体 9.8 m 的一半 ✓） | 网格 `Body` ✓ |
| **germ_leopard_2a4** | **15** ✓ | **拒收** ✓（`the mesh is a plate/detail shell, not a hull volume; ring z-spans are slivers of the 7.461 m overall span` ✓） | 网格 `HullArmour` ✓ |

### 改进
1. **固定容差 → 自适应最近顶点带** ✓：固定容差在薄壳网格上会整带落空 ✗（豹2 中环曾为空 ✓）；现在保证取到真实截面 ✓，并**逐环记录实际 y 容差** ✓（自审计 ✓：T-80B 0.259/0.019/0.263 m ✓、豹2 0.040/0.120/0.004 m ✓）；
2. **薄片拒收** ✓：环的 z 跨度 < 整体跨度的 50% ⇒ **整组拒收并给出实测跨度** ✓ ⇒ 豹2 的环**带理由地缺席** ✓（而不是"看着合理的 4 cm 薄片" ✗）。

### 认知修正（可复用的教训 ✓）
- 我先后用**名字**判断豹2 是否有车体网格 ✗（先说没有 ✗、后说有 ✓）—— **两次都靠不住** ✗；
- **几何**给出答案 ✓：网格**存在** ✓，但它**不是车体体积** ✗（稀疏装甲板 ✓）⇒ **结论必须由几何支撑，不能由命名支撑** ✓；
- ⇒ 豹2A4 的 `hull_rings` **无法从该产物测得** ✓ ⇒ 归入**作者/设计项** ✓（与历史包"**尺寸为手写估计值**（`build_historical_packets.py` ✓）"的性质一致 ✓✓）。

---

## 7. A 类验收自校验（29/29 PASS ✓）`tests/check_modern_geometry.gd`
**刻意不做"工具自我比对"** ✗，改为三项**互相独立**的检查 ✓：

| 校验 | 内容 | 结果 |
|---|---|---|
| **A. 适配产物 vs 源模型** | 同一批关键测量（车体 AABB ✓、炮塔/火炮枢轴 ✓、炮塔上下沿 ✓、负重轮数 ✓）在两份文件上应一致（容差 1 mm ✓） | **全一致（小数点后 4 位 ✓）** ⇒ **导出未改动几何** ✓✓ |
| **B. 内部不变量** | 每个测量字段**必须有 `method`** ✓；3 环且高度递增 ✓、半宽为正 ✓、前 z<后 z ✓；`turret_bottom<turret_top` ✓；`turret_taper` 合理 ✓ | 全通过 ✓（未解释字段 = 0 ✓） |
| **C. 确定性** | 草案 JSON 中的值 == 现场重测 ✓ | 通过 ✓ |

**为什么 A 是关键** ✓：适配产物是模型的**导出** ✓，若导出挪动了几何 ✓，**所有**实测字段都会被污染 ✗ ——
本会话早前的"标记变换"事故正是这一类 ✓；现在它被**永久断言**看住 ✓。

**附带** ✓：校验独立测得豹2A4 的 hull AABB **z 跨度 7.461 m** ✓，与拒收注记逐字吻合 ✓ ⇒ 薄片结论获**第二处独立证据** ✓。
