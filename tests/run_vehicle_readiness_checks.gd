...
func _run() -> void:
	...
	_check(int(summary.get("tree_rows_with_model",0)) == 141,"141 tree rows have a model")
	_check(int(summary.get("research_model_glb_count",0)) == 141,"141 model GLB files exist in the repository")
	_check(int(summary.get("tree_variant_refs",0)) == 160,"160 variant references are recorded")
	_check(int(summary.get("tree_rows_with_combat_package",0)) == 2,"two tree vehicles have exact combat-package bindings")
	_check(int(summary.get("tree_admission_status",{}).get("combat_engineering",0)) == 2,"two rows are engineering-combat aligned")
	_check(int(summary.get("tree_admission_status",{}).get("static_preview_only",0)) == 139,"139 modeled rows remain static_preview_only")
	_check(int(summary.get("tree_admission_status",{}).get("awaiting_model",0)) == 71,"71 rows await a model")
	_check(int(summary.get("admitted_combat_vehicles",0)) == 6,"four historical and two engineering vehicles are combat admitted")
