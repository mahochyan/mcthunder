	...
	for i in 20000:
		...
		for range_m in ranges:
			if not out.has(range_m) and projectile.position_world.x >= range_m:
				...
				var speed: float = previous.lerp(projectile.position_world,alpha).length()
				out[range_m] = {"speed": projectile.velocity_world.length(), "x": projectile.position_world.x, "alpha": alpha, "distance": speed}
		if out.size()==ranges.size(): break
	manager.despawn(spawned.projectile_id)
...
func _run() -> void:
	...
	for row in table:
		...
		worst_control = maxf(worst_control,absf(1.0-float(row_control.get("distance",0.0))/MUZZLE_MPS))
	...
	for row in table:
		...
		var retention: float = float(row_measured.get("distance",0.0))/MUZZLE_MPS
