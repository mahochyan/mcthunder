...
$run=Join-Path $projectRoot "logs/WT040-package/$SourceSha/$Case-$(Get-Date -Format yyyyMMdd-HHmmss-fff)"
...
$eventFile=$null
if($Case -eq 'life'){
    $eventFile=Get-ChildItem -LiteralPath "$run/userdata" -Filter 'evidence.json' -Recurse | Select-Object -First 1
    if($eventFile){
