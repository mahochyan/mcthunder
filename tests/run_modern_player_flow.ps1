...
$run=Join-Path $root "logs/WT040-modern-player/$SourceSha/$(Get-Date -Format yyyyMMdd-HHmmss-fff)"
New-Item -ItemType Directory -Force -Path "$run/workdir","$run/userdata" | Out-Null
function Run-Phase([string]$Name,[string]$Extra,[int]$Limit){
    ...
    $psi.EnvironmentVariables['APPDATA']="$run/userdata"
...
$saveDir="$run/userdata/Godot/app_userdata/PixelArmor/tests/modern_match_current"
if($match.passed){
...
$shots="$run/userdata/Godot/app_userdata/PixelArmor/tests/modern_match_shots"
