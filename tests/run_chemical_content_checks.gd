...
func _shell(index: int) -> Dictionary:
	...
	data.label="TEST ONLY authored HEAT"; data.family="HEAT"
	data.source_bullet_type="heat_fs_tank"; data.effect_policy="chemical"
	data.impact_profile=HeatFixture.impact()
	data.chemical_profile=HeatFixture.profile()
	data.penetration_curve=[[0,100],[1000,100]]
	data.evidence.ballistics.value.penetration_curve=data.penetration_curve.duplicate(true)
	...
	data.evidence.chemical=_claim(data.chemical_profile.duplicate(true),"structured")
	for key in ["impact","chemical"]:
		data.evidence[key].source_refs=["chemical_fixture"]
		data.evidence[key].location="tests/fixtures/chemical_profile.gd"
	return data
...
func _fixture(count: int = 1) -> Dictionary:
	...
	source.artifact="res://tests/fixtures/chemical_profile.gd"; source.sha256=FileAccess.get_sha256(source.artifact)
	packet.sources.chemical_fixture=source
	return packet

func _run() -> void:
	var packet := _fixture(2)
	packet.sources.fixture.sha256=FileAccess.get_sha256("res://tests/run_chemical_content_checks.gd")
	packet.sources.fixture.artifact="res://tests/run_chemical_content_checks.gd"
	var built := VehicleShellCatalog.build(packet)
	check(built.ok and built.options.size()==2,"explicit game-rule AP and HEAT enter one typed catalog")
	if not built.ok: print(built.errors); quit(1); return
	check(built.options[1].validate().ok and built.options[1].effect_policy=="chemical" and built.options[1].impact_profile.family=="HEAT","admitted definition retains separate chemical policy")
	for defect in ["missing_profile","missing_evidence","changed_material","fake_source","ap_relabel","old_profile","fuze","missing_curve","null_evidence","missing_chemical","changed_chemical","missing_chemical_evidence","declining_curve"]:
		...
		match defect:
			...
			"changed_material": shell.impact_profile.material_coefficients.rolled=1.8
			...
			"missing_chemical": shell.erase("chemical_profile")
			"changed_chemical": shell.chemical_profile.path_loss_mm_per_m=20
			"missing_chemical_evidence": shell.evidence.erase("chemical")
			"declining_curve": shell.penetration_curve[1][1]=90; shell.evidence.ballistics.value.penetration_curve=shell.penetration_curve.duplicate(true)
		check(not VehicleShellCatalog.build(bad).ok,"reject incomplete or mislabeled HEAT: "+defect)
	...
	await _frames(20)
	check(gun.select_shell(1) and gun.inventory.chamber_shell.ends_with("_shell"),"select HEAT while retaining chambered AP")
	...
	check(gun.shell.effect_policy=="chemical" and gun.inventory.chamber_shell.ends_with("fixture_shell_1"),"ordinary reload physically chambers selected HEAT")
	var muzzle := actor.turret.muzzle.global_position
	check(actor.submit_command(command),"normal command submits HEAT shot")
	await _frames(2)
	var rod := manager.get_projectile_state(gun.last_projectile_id)
	check(rod!=null and rod.effect_policy=="chemical" and rod.impact_profile==HeatFixture.impact(),"actual launch freezes separate HEAT response")
	check(rod!=null and rod.launch_position.distance_to(muzzle)<0.02 and is_equal_approx(rod.launch_velocity.length(),620.0),"HEAT launch uses actual muzzle and its own authored speed")
	...
	if rod!=null:
		...
		var target := ArmorTrainingTargets.build([
			...
			{"center":rod.launch_position+direction*6.0,"thickness":200}],"chemical_target",1)
		...
		check(rod.contacts.size()==2 and rod.contacts[0].result=="penetrated" and rod.terminal_reason=="chemical_detonation" and rod.chemical_effect.reason=="armor_stopped","normally fired HEAT penetrates actual first plate and stops at second")
	...
	print("CHEMICAL_CONTENT_CHECKS_PASS" if failures==0 else "CHEMICAL_CONTENT_CHECKS_FAIL")
	quit(0 if failures==0 else 1)

func inspect_modern_shot(rod: ProjectileState, record: Dictionary) -> void:
	check(rod.chemical_profile==HeatFixture.profile() and rod.chemical_effect.complete,"normal Gunner launch freezes separate chemical configuration and completes jet")
	check(rod.travelled_m<5.0 and rod.burst.is_empty() and rod.fragments.is_empty(),"normal HEAT carrier stops at first plate and uses a separate jet")
	check(record.rules_versions.get("chemical")==ChemicalProfile.VERSION,"normal HEAT replay preserves the exact chemical rules version")
