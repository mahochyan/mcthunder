...
func capture(name: String) -> void:
	...
	check(root.get_texture().get_image().save_png(shot_dir.path_join(name+".png"))==OK,"capture actual modern frontend: "+name)
func run() -> void:
	create_timer(180,true,false,true).timeout.connect(func() -> void: print("MODERN_GARAGE_TIMEOUT"); quit(2))
	var save_path := "user://tests/modern_garage_%d/profile" % Time.get_ticks_usec()
	...
	app.profile = ProfileStore.new(save_path)
