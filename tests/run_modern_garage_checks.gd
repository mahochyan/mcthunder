...
func run() -> void:
	...
	app.profile=ProfileStore.new("user://tests/modern_garage_%d/profile" % Time.get_ticks_usec())
	root.add_child(app); current_scene=app
	var verifier: Node=load("res://scripts/diagnostics/modern_garage_verifier.gd").new()
