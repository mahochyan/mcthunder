...
func _damage_cases() -> void:
	...
	b.reset_vehicle()
	var mover := str(b.state.crew_assignments.get("commander",""))
	print("[T02 leg] damage before: mover=%s assignments=%s alive=%s" % [mover,str(b.state.crew_assignments),str((b.state.crew_states.get(mover,{}) as Dictionary).get("alive",""))])
	_ok(b.state.assign_crew("driver",mover),"living person can occupy another role")
	print("[T02 leg] damage after : assignments=%s" % str(b.state.crew_assignments))
...
func _rotated_and_runtime_cases() -> void:
	...
	var rotated := str(b.state.crew_assignments.get("gunner",""))
	print("[T02 leg] rotated: person=%s assignments=%s" % [rotated,str(b.state.crew_assignments)])
