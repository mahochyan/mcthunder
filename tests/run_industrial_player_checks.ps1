param([int]$TimeoutSeconds = 1200,[string]$EvidenceTag = '')
...
$logs = Join-Path $projectRoot "logs/023/$folder/$stamp"
$shots = Join-Path $projectRoot "docs/evidence/023/$folder/$stamp"
New-Item -ItemType Directory -Force -Path $logs,$shots | Out-Null
$argsText = '--path "'+$projectRoot+'" --fixed-fps 60 --disable-vsync -- --industrial-play-check --shot-dir "'+$shots+'"'
$out = Join-Path $logs 'industrial_stdout.log'
$err = Join-Path $logs 'industrial_stderr.log'
...
$passed = -not $timedOut -and $p.ExitCode -eq 0 -and $marker.Success -and [int]$marker.Groups[1].Value -eq 34 -and [int]$marker.Groups[2].Value -eq 0 -and ($output+$errors) -notmatch '(?m)^ERROR:|SCRIPT ERROR:|Parse Error:'
...
if (-not $passed -or $images.Count -ne 11) { exit 1 }
exit 0
