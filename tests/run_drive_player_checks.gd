...
func _run() -> void:
	...
	await _frames(55)
	_check(scene.actor.tank.chassis.pitch>0,"normal W produces hull acceleration pitch")
	await _key(KEY_W,false)
	await _key(KEY_S,true)
	await _frames(10)
	_check(scene.actor.tank.chassis.pitch<0,"normal S while moving forward produces braking pitch")
	await _capture("02c_braking_pitch")
	await _key(KEY_S,false)
	await _frames(180)
	_check(absf(scene.actor.tank.chassis.pitch)<0.0001,"released keys allow braking attitude to settle")
	await _tap(KEY_1)
	await _key(KEY_W,true)
	await _frames(190)
	await _key(KEY_W,false)
